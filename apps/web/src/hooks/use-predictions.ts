/**
 * Predictions query + mutation hooks.
 *
 * Port literal de `apps/mobile/src/hooks/use-predictions.ts`. Mantiene los
 * MISMOS query keys, los MISMOS nombres de hook y la MISMA lógica de
 * invalidación que mobile — esto es requisito de paridad para que las
 * reglas de `use-socket-events` y los downstream features funcionen
 * idénticamente en ambos clientes.
 *
