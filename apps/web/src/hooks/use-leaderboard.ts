/**
 * Leaderboard hooks — port literal de `apps/mobile/src/hooks/use-leaderboard.ts`.
 *
 * `useLeaderboard(groupId)` es lo que consumen las pantallas de grupo;
 * `useMyPosition` y `useGlobalLeaderboard` quedan disponibles para futuros
 * usos manteniendo paridad con mobile.
 */
 ...
 *
 * `currentUserEntry` is extracted from the first page response and stays
 * stable across page fetches.
 */
