...
/**
 * Port literal de `apps/mobile/src/api/groups.ts`.
 *
...
export async function leaveGroup(id: string): Promise<{ message: string }> {
  const { data } = await api.post<{ message: string }>(`/groups/${id}/leave`);
  return data;
}

export async function getMembers(groupId: string): Promise<GroupMemberDto[]> {
